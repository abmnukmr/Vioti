...
export class PhoneverPage {
 ...
 loading:Loading;
  ...
  num:any;
  name:any;
  constructor(public loadingCtrl:LoadingController,public http:Http,public navCtrl: NavController, public navParams: NavParams, public formBuilder: FormBuilder,public toastCtrl: ToastController) {
  ...
  updatedata() {
    ...
    if (this.registerForm.value.phone == null || this.registerForm.value.phone == '') {
      ...
      headers.append('Access-Control-Allow-Origin', '*');
      let options = new RequestOptions({headers: headers});
    //https://vioti.herokuapp.com/user/create/otp/'
      console.log("done otppp sending...");
      this.http.post('https://vioti.herokuapp.com/user/create/otp/' + this.email1, JSON.stringify(this.update), options)
